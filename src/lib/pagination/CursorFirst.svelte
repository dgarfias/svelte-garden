<!--
  Svelte Garden - CursorFirst Component
  
  First page button for cursor pagination.
-->
<script lang="ts">
...
<button
  type="button"
  class="garden-cursor-btn garden-cursor-btn--first {className}"
  class:garden-cursor-btn--disabled={disabled}
  class:garden-cursor-btn--rtl={isRtl}
  data-garden-id="pagination.cursor.first"
  {disabled}
  onclick={onClick}
...
>
  ...
  {:else}
    <svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
      <path d="M11 13.5L5.5 8 11 2.5M4 2.5v11" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
  {/if}
</button>

<!--
  Svelte Garden - CursorPrevious Component
...
<script lang="ts">
  ...
  import { ChevronLeftStroke } from '../icons';
...
<button
  type="button"
  class="garden-cursor-btn garden-cursor-btn--previous {className}"
  class:garden-cursor-btn--disabled={disabled}
  class:garden-cursor-btn--rtl={isRtl}
  data-garden-id="pagination.cursor.previous"
  {disabled}
  onclick={onClick}
  aria-label="Previous"
  {...restProps}

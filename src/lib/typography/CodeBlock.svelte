...
<div class="garden-codeblock-container">
  ...
  <pre class={codeBlockClasses} data-garden-id="typography.codeblock" {...restProps}><code>{#each lines as line, lineIndex}{@const lineNum = lineIndex + 1}<span class="line" class:line--highlighted={highlightLines.includes(lineNum)} data-line={lineNum}><span class="line-content">{#each line as token}{#if typeof token === 'string'}{token}{:else}<span class={getTokenClasses(token)}>{getTokenContent(token)}</span>{/if}{/each}{#if line.length === 0} {/if}</span></span>{/each}</code></pre>
</div>
